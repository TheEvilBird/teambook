...
int get_siz(pnode v) { return (v == nullptr ? 0 : v->siz); }

int get_min(pnode v) { return (v == nullptr ? INF : v->mn); }
...
bool is_root(pnode v) {
  return (v->par == nullptr || (v->par->ch[0] != v && v->par->ch[1] != v));
}

int child_num(pnode v) { return (v->par->ch[1] == v); }
