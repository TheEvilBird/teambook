 ...
 * Date: ?
 * Description: Segment Tree beats (TODO: WRITE OPERATIONS).
 */
...
struct SegTree {
  ...
  void recalc_min(int v, int l, int r, ll cur) {
    ...
    tree[v].sum -= (ll)(tree[v].max - cur) * (ll)(tree[v].cnt_max);
    tree[v].max = cur;
  }
  ...
  void build_tree(int v, int l, int r) {
    if (l + 1 == r) {
      tree[v] = {a[l], -INFLL, 1, a[l], INFLL, 1, a[l], 0, -1};
      return;
    }
    int m = (l + r) / 2, vL = 2 * v, vR = vL + 1;
    build_tree(vL, l, m);
  ...
  void update_segment_min_tree(int v, int l, int r) {
    ...
    int m = (l + r) / 2, vL = 2 * v, vR = vL + 1;
    if (qL < m)
      update_segment_min_tree(vL, l, m);
    if (m < qR)
      update_segment_min_tree(vR, m, r);
    update_vertex(v, l, r);
  }
  ...
  void update_segment_max_tree(int v, int l, int r) {
    ...
    int m = (l + r) / 2, vL = 2 * v, vR = vL + 1;
    if (qL < m)
      update_segment_max_tree(vL, l, m);
    if (m < qR)
      update_segment_max_tree(vR, m, r);
    update_vertex(v, l, r);
  }
  ...
  void update_segment_add_tree(int v, int l, int r) {
    ...
    int m = (l + r) / 2, vL = 2 * v, vR = vL + 1;
    if (qL < m)
      update_segment_add_tree(vL, l, m);
    if (m < qR)
      update_segment_add_tree(vR, m, r);
    update_vertex(v, l, r);
  }
