...
template <typename T> struct point_t {
  ...
  T operator*(const point_t<T> &other) const {
    return x * other.x + y * other.y;
  } // dot product
  T operator%(const point_t<T> &other) const {
...
template <typename T> istream &operator>>(istream &is, point_t<T> &vec) {
  is >> vec.x >> vec.y;
  return is;
}

template <typename T> ostream &operator<<(ostream &os, const point_t<T> &vec) {
  os << vec.x << ' ' << vec.y;
  return os;
}
