...
PointLD hp_point(const Half &h) {
  return {h[0], h[1]};
} // direction of half-plane
PointLD isect(const Half &h0,
              const Half &h1) { // Cramer's rule to intersect half-planes
  array<ld, 3> vals{};
  for (int i = -1; i <= 1; ++i) {
...
ld x_isect(const Half &h0, const Half &h1) {
  return isect(h0, h1).x;
} // x-coordinate of intersection

vH construct_lower(PointLD x,
                   vH planes) { // similar to convex hull (by duality)
  ...
  auto lst_x = [&](Half a, Half b) {
    if (fabsl(hp_point(a) % hp_point(b)) <=
        EPS) { // parallel half-planes, remove lower one
  ...
  for (auto t : planes) {
    while (sz(res) > 1 &&
           lst_x(res.back(), t) <= lst_x(res[sz(res) - 2], res.back())) {
      res.pop_back();
    }
    res.push_back(t);
  }
...
ld isect_area(vH planes) {
  ...
  while (iu < sz(upper) - 1 &&
         il < sz(lower) -
                  1) { // sweep vertical line through lower and upper hulls
...
Half plane_through(PointLD p,
                   PointLD dir) { // half-plane through p in direction dir
