...
bool point_on_line(const Point &p, const Point &a,
                   const Point &b) { // point p, line ab
  Point ab = b - a, ap = p - a;
  return (ab % ap) == 0;
}

bool point_on_ray(const Point &p, const Point &a,
                  const Point &b) { // point p, ray ab
  Point ab = b - a, ap = p - a;
  return ((ab % ap) == 0 && (ab * ap) > 0) || (a == p);
}

bool point_on_segment(const Point &p, const Point &a,
                      const Point &b) { // point p, segment ab
  Point ap = p - a, bp = p - b, ab = b - a;
  return (ap * bp) <= 0 && (ap % ab) == 0;
}

bool point_in_angle(const Point &p, const Point &a, const Point &o,
                    const Point &b) { // point p, angle aob
...
bool segment_intersection(const Point &a, const Point &b, const Point &c,
  ...
  if (get_sign((ab % ac)) * get_sign((ab % ad)) <= 0 &&
      get_sign((cd % ca)) * get_sign((cd % cb)) <= 0) {
    ll x1 = max(min(a.x, b.x), min(c.x, d.x)),
       x2 = min(max(a.x, b.x), max(c.x, d.x));
    ll y1 = max(min(a.y, b.y), min(c.y, d.y)),
...
bool rays_intersection(const Point &a, const Point &b, const Point &c,
  ...
  if (line_ray_intersection(a, b, c, d) && line_ray_intersection(c, d, a, b)) {
    ...
    ll x1 = max(min(a.x, bp.x), min(c.x, dp.x)),
       x2 = min(max(a.x, bp.x), max(c.x, dp.x));
    ll y1 = max(min(a.y, bp.y), min(c.y, dp.y)),
       y2 = min(max(a.y, bp.y), max(c.y, dp.y));
    return (x1 <= x2 && y1 <= y2);
  }
  return false;
}

bool segment_line_intersection(const Point &a, const Point &b, const Point &c,
...
bool ray_segment_intersection(const Point &a, const Point &b, const Point &c,
                              const Point &d) { // ray ab, segment cd
  if (line_ray_intersection(c, d, a, b) &&
      segment_line_intersection(c, d, a, b)) {
    Point bp = get_inf(a, b);
    ll x1 = max(min(a.x, bp.x), min(c.x, d.x)),
       x2 = min(max(a.x, bp.x), max(c.x, d.x));
    ll y1 = max(min(a.y, bp.y), min(c.y, d.y)),
       y2 = min(max(a.y, bp.y), max(c.y, d.y));
    return (x1 <= x2 && y1 <= y2);
  }
  return false;
}